...
if ($_REQUEST['do'] == 'add' OR $_REQUEST['do'] == 'edit')
{
	...
	else
	{
		...
		$sub['cost'] = vb_unserialize($sub['cost']);
	...
	//cache_forums();
	if ($old_sub_masks = vb_unserialize($sub['forums']) AND is_array($old_sub_masks))
	{
...
if ($_REQUEST['do'] == 'adjust')
{
	...
	else
	{
		...
		$cost_length = vb_unserialize($subinfo['cost']);
...
if ($_REQUEST['do'] == 'apiedit' OR $_REQUEST['do'] == 'apiadd')
{
	...
	{
		$settings = vb_unserialize($api['settings']);
...
if ($_POST['do'] == 'apiupdate')
{
	...
	if (!empty($vbulletin->GPC['paymentapiid']) AND !empty($vbulletin->GPC['settings']))
	{
		...
		$settings = vb_unserialize($currentinfo['settings']);
...
if ($_REQUEST['do'] == 'transdetails')
{
	...
	$request = vb_unserialize($payment['request']);
	...
	else
	{
		...
		if ($get = vb_unserialize($request['GET']))
		{
		...
		if ($post = vb_unserialize($request['POST']))
		{
	...
	if ($vbulletin->GPC['type'] == 'stats')
	{
		if ($vbulletin->GPC['scope'])
		{
			...
			$style = $db->query_first("
				...
				LIMIT 1
			");
			$stylevars = vb_unserialize($style['stylevars']);
			unset($style);
