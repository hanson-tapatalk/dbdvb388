...
?>
<script type="text/javascript" src="../clientscript/vbulletin_cpcolorpicker.js?v=<?php echo $vbulletin->options['simpleversion'];?>"></script>
<?php
...
if ($_REQUEST['do'] == 'doedit')
{
	...
	{
		$templateids = implode(',', vb_unserialize($stylecache[$vbulletin->GPC['dostyleid']]['templatelist']));
		if (!$templateids)
		{
			// this used to cause an SQL error, this should work as an alternative
			$templateids_query = "styleid = " . $vbulletin->GPC['dostyleid'];
	...
	{
		$css["$title"] = vb_unserialize($template['template']);
...
if ($_REQUEST['do'] == 'stylevar-colors')
{
	...
	))
	{
		...
		foreach (vb_unserialize($style['csscolors']) AS $colorname => $colorvalue)
		{
...
if ($_REQUEST['do'] == 'moo')
{
	...
	{
		print_label_row(htmlspecialchars_uni($l), "<span class=\"smallfont\">$r</span>");
	}
