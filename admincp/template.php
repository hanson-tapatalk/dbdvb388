...
if ($_REQUEST['do'] == 'download')
{
	...
	else
	{
		...
		if ($vbulletin->GPC['mode'] == 1)
		{
			...
			$sqlcondition = "styleid <> -1 AND templateid IN(" . implode(',', vb_unserialize($style['templatelist'])) . ")";
		}
		else
		{
...
if ($_POST['do'] == 'replace')
{
	...
	if ($editmaster != true)
	{
		...
		$templatelist = vb_unserialize($styleinfo['templatelist']);
	}
	else
	{
		$styleinfo = array(
...
if ($_POST['do'] == 'inserttemplate')
{
	...
	if (empty($exists[$vbulletin->GPC['dostyleid']]))
	{
		/*insert query*/
