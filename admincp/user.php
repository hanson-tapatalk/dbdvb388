...
define('CVS_REVISION', '$RCSfile$ - $Revision: 93801 $');
...
if ($_REQUEST['do'] == 'find')
{
	...
	{
		$vbulletin->GPC['user']    = vb_unserialize(verify_client_string($vbulletin->GPC['serializeduser']));
		$vbulletin->GPC['profile'] = vb_unserialize(verify_client_string($vbulletin->GPC['serializedprofile']));
	}

	if (!empty($vbulletin->GPC['serializeddisplay']))
	{
		$vbulletin->GPC['display'] = vb_unserialize(verify_client_string($vbulletin->GPC['serializeddisplay']));
	}
...
if ($_POST['do'] == 'update')
{
	...
	$olduser = vb_unserialize(verify_client_string($vbulletin->GPC['olduser']));
		...
		while ($profilefield = $db->fetch_array($profilefields))
		{
			...
			if ($vbulletin->GPC['display']["field$profilefield[profilefieldid]"])
			{
				...
				if ($profilefield['type'] == 'checkbox' OR $profilefield['type'] == 'select_multiple')
				{
					...
					{
						if (intval($user[$profilefieldname]) & (2**$index))
						{
							if (!empty($output))
							{
								$output .= '<b>,</b> ';
