...
if ($_POST['do'] == 'dodeletespam')
{
	...
	{ // report to Akismet
		...
		while ($post = $db->fetch_array($posts))
		{
			$akismet->mark_as_spam(array('user_ip' => $post['ipaddress'], 'user_agent' => $post['useragent'], 'comment_type' => 'post', 'comment_author' => $post['username'], 'comment_content' => $post['pagetext']));
		}
	}
...
if ($_POST['do'] == 'domergethreads')
{
	...
	$threadIds = array_keys($threadarray);
	...
	$db->query_write("
		...
		WHERE open = 10
			AND pollid IN(" . implode(',', $threadIds) . ")
	");
...
if ($_POST['do'] == 'approveposts')
{
	...
	$thread_ids = array_keys($threadlist);

	foreach ($thread_ids AS $threadid)
	{
		build_thread_counters($threadid);
	}
	foreach (array_keys($forumlist) AS $forumid)
			...
			if ($vbulletin->options['attachfile'])
			{
				...
				if (vbmkdir($new_path))
				{
					...
					while ($attachment = $db->fetch_array($attachments))
					{
						...
						if (@copy("$old_path/$attachment[attachmentid].attach", "$new_path/$attachment[attachmentid].attach"))
						{
							@unlink("$old_path/$attachment[attachmentid].attach");
						}

						if (file_exists("$old_path/$attachment[attachmentid].thumb"))
						{
							if (@copy("$old_path/$attachment[attachmentid].thumb", "$new_path/$attachment[attachmentid].thumb"))
							{
								@unlink("$old_path/$attachment[attachmentid].thumb");
							}
						}
					}
				}
