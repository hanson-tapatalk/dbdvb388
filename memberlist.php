...
if ($_REQUEST['do'] == 'getall')
{
	...
	while ($profilefield = $db->fetch_array($profilefields))
	{
		...
		if ($profilefield['memberlist'])
		{
			...
			{
					$profilefield['data'] = vb_unserialize($profilefield['data']);
			}
	...
	if ($show['agecol'])
	{
		$agecondition = ', IF(YEAR(user.birthday_search) > 0 AND user.showbirthday IN (1,2) AND YEAR(user.birthday_search) < YEAR(CURDATE()), user.birthday_search, \'1000-01-01\') AS agesort';
	}
	else
	{
		$agecondition = '';
			...
			foreach ($profileinfo AS $index => $value)
			{
				if ($userinfo["$value[varname]"] != '')
				{
					if ($value['type'] == 'checkbox' OR $value['type'] == 'select_multiple')
					{
						...
						{
							if (intval($userinfo[$value[varname]]) & (2**$key))
							{
