...
function mobiquo_chop($string)
{
     ...
     */
    $string = preg_replace('/\[quote(.*?)\](.*?)\[\/quote\]/si','[quote]',$string);
    $string = preg_replace('/\[url(.*?)\](.*?)\[\/url\]/si','[url]',$string);
    $string = preg_replace('/\[img(.*?)\](.*?)\[\/img\]/si','[img]',$string);
    $string = preg_replace('/\[code(.*?)\](.*?)\[\/img\]/si','[code]',$string);
    $string = preg_replace('/\[video=(.*?)\](.*?)\[\/video\]/si','[V]',$string);
    $string = preg_replace('/\[attach\](.*?)\[\/attach\]/si','[attach]',$string);
    $string = preg_replace('/\[vimeo(.*?)\](.*?)\[\/vimeo\]/si','[vimeo]',$string);
    $string = preg_replace('/\[spoiler(.*?)\](.*?)\[\/spoiler\]/si','[spoiler]',$string);
    $string = preg_replace('/\[youtube(.*?)\](.*?)\[\/youtube\]/si','[youtube]',$string);
...
function post_content_clean($str, $return_html = false)
{
    ...
    $str = preg_replace('#\[vimeo\]([^\[]+)\[/vimeo\]#sieU', "process_vimeo('$1')", $str);
    ...
    //$str = clean_quote($str);
    $str = preg_replace('/\[quote=(?!(\'|"))([^\]]*?)\]/sei', "process_quote_name('$2')", $str);
    $str = preg_replace('/\[quote=(\'|")(.{0,30}?;\s*(\d+)\s*)\1\]/sei', "process_quote_name('$2')", $str);
...
function register_user($need_email_verification = false, $gravatar = false, $tid_profile = array(),$verified = false)
{
        ...
        return 'Password cannot be empty';
    {
        ...
        $birthdaySet = false;
        if($vbulletin->options['usecoppa'] && isset($vbulletin->GPC['month']) && isset($vbulletin->GPC['year']) && isset($vbulletin->GPC['day']))
        {
            $birthday = array(
                   'day'   => $vbulletin->GPC['day'],
                   'month' => $vbulletin->GPC['month'],
