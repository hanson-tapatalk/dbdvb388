...
function get_online_users_func($params)
{
    ...
    while ($loggedin = $db->fetch_array($forumusers))
    {
        ...
        else if (empty($userinfos[$userid]) OR ($userinfos["$userid"]['lastactivity'] < $loggedin['lastactivity']))
        {
            ...
            $userinfos[$userid]['musername'] = $loggedin['musername'];
            $userinfos[$userid]['useragent'] = htmlspecialchars_uni($loggedin['useragent']);
            construct_im_icons($userinfos[$userid]);
            ...
            {
                $userinfos[$userid]['host'] = @gethostbyaddr($loggedin['host']);
            }
    ...
    if (is_array($userinfos))
    {
        foreach ($userinfos AS $key => $val)
        {
            if (!$val['invisible'])
            {
                $userinfos[$key] = process_online_location($val, 1);
            }
        }
    }
