...
function get_moderate_topic_func($xmlrpc_params)
{
    ...
    if (empty($modforums))
    {
        return new xmlrpcresp(new xmlrpcval( array(
            ...
            'is_login_mod' => new xmlrpcval(true, 'boolean'),
            'result_text' =>  new xmlrpcval('security error (user may not have permission to access this feature)', 'base64')
        ), 'struct'));
    }
    ...
    if ($totalthreads)
    {
        ...
        while ($thread = $db->fetch_array($threads))
        {
            ...
            //    if($show['moderated'] == true && $show['spam'] == false && $show['deletedthread'] == false){
            ...
            $return_topic = new xmlrpcval(array(
                ...
                'can_move'          => new xmlrpcval($mobiquo_can_move, 'boolean'),
