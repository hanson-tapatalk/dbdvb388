...
*
* @package	vBulletin
* @version	$Revision: 93992 $
* @date		$Date: 2017-05-10 05:18:19 -0700 (Wed, 10 May 2017) $
*/
class vB_DataManager_ThreadPost extends vB_DataManager
{
	...
	*/
	function verify_title(&$title)
	{
		...
		if ($this->registry->options['titlemaxchars']
			AND (empty($this->existing['title']) OR $title != $this->existing['title']))
		{
	...
	*/
	function insert_postlog_data()
	{
		if (empty($this->info['is_automated']))
		{
			...
			$this->dbobject->query_write("
				...
				VALUES
				(" . $postid . ", '" . $this->dbobject->escape_string(USER_AGENT) . "', '" . IPADDRESS . "', " . TIMENOW . ")
			");
		}
	}
	...
	function akismet_mark_as_ham($postid)
	{
		...
		$postdata = $this->dbobject->query_first("
			SELECT post.username AS username, post.pagetext AS pagetext, 
				postlog.ipaddress AS ip, postlog.useragent AS useragent 
...
class vB_DataManager_Thread extends vB_DataManager_ThreadPost
{
	...
	*/
	...
	function pre_save($doquery = true)
	{
		...
		if (!empty($this->thread['username']))
		{
			$this->do_set('postusername', $this->thread['username']);
		}
		if (!empty($this->thread['userid']))
		{
		...
		if (!$this->condition)
		{
			...
			{
				$oldvalue = empty($this->info['skip_moderator_log']) ? NULL : $this->info['skip_moderator_log'];
...
class vB_DataManager_Thread_FirstPost extends vB_DataManager_Thread
{
	...
	*/
	...
	function post_save_each($doquery = true)
	{
		...
		if ($this->info['mark_thread_read'] AND $this->info['forum'] AND $userid = $this->fetch_field('postuserid'))
		{
			$threadinfo = fetch_threadinfo($threadid);
			if ($threadinfo)
			{
