...
function log_vbulletin_error($errstring, $type = 'database')
{
	...
	switch($type)
	{
		...
		case 'php':
			...
			{
				$errfile = trim($vbulletin->options['errorlogphp']);
		...
		case 'database':
			...
			{
				$errfile = trim($vbulletin->options['errorlogdatabase']);
		...
		case 'security':
			...
			{
				$errfile = trim($vbulletin->options['errorlogsecurity']);
				$username = $errstring;
	...
	if (empty($errfile) OR (defined('DEMO_MODE') AND DEMO_MODE == true))
	{
		return false;
	}
