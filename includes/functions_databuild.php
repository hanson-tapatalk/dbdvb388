...
function build_thread_counters($threadid)
{
	...
	($hook = vBulletinHook::fetch_hook('threadcounters_ratings_query')) ? eval($hook) : false;
	...
	($hook = vBulletinHook::fetch_hook('build_thread_counters')) ? eval($hook) : false;
...
function build_user_statistics()
{
	...
	{
		$sumsql = "SUM(IF(lastactivity >= " . (TIMENOW - $vbulletin->options['activememberdays'] * 86400) . ", 1, 0)) AS active,";
	}
	else
	{
		$sumsql = '';
	...
	$values = array(
		...
		'activemembers' => empty($members['active']) ? NULL : $members['active'],
...
function fetch_phrase_group($groupname)
{
	...
	$vbphrase = array_merge($vbphrase, vb_unserialize($group["$groupname"]));
}
