	...
	while (++$rows <= 6)
	{
		...
		foreach ($userweek AS $daycount => $curday)
		{
			...
			else
			{
				...
				if ($calendarinfo['showbirthdays'] AND $fullcalendar AND !empty($birthdaycache["$month"]["$day"]) AND is_array($birthdaycache["$month"]["$day"]))
				{
					unset($userday);
					unset($age);
					$birthdaydesc = '';
...
function cache_events($range)
{
		...
		if ($vbulletin->db->num_rows($holidays))
		{
			while ($ev = $vbulletin->db->fetch_array($holidays))
			{
				...
				else
				{
					...
					$offset = intval($vbulletin->userinfo['timezoneoffset'] ? $vbulletin->userinfo['tzoffset'] : $vbulletin->userinfo['timezoneoffset']);
	...
	if ($vbulletin->db->num_rows($events))
	{
		while ($ev = $vbulletin->db->fetch_array($events))
		{
			...
			$offset = intval($ev['dst'] ? $vbulletin->userinfo['timezoneoffset'] : $vbulletin->userinfo['tzoffset']);
			$ev['dateline_from_user'] = $ev['dateline_from'] + $offset * 3600;
			$ev['dateline_to_user'] = $ev['dateline_to'] + $offset * 3600;
			fetch_musername($ev);
