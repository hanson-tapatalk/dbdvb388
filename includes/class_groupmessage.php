...
class vB_Collection
{
	...
	/**
	 * Calculate total via SQL_CALC_FOUND_ROWS
	 *
	 * @access protected
	 * @var bool
	 */
	 ...
	 */
	function __construct(&$registry, $parent_id = false, $page = 1, $quantity = false, $descending = true, $no_limit = false)
	{
...
class vB_Group_Collection_Factory extends vB_Collection_Factory
{
	 ...
	 */
	function __construct(&$registry, $group = false)
	{
...
class vB_Group_Collection extends vB_Collection
{
	 ...
	 * @return vB_Group_Collection
	 */
	function __construct(&$registry, $group = false, $parent_id = false, $page = 1, $quantity = false, $descending = true, $no_limit = false)
	{
		parent::__construct($registry, $parent_id, $page, $quantity, $descending, $no_limit);
...
class vB_Group_Collection_Message extends vB_Group_Collection
{
	...
	function collection_sql($force_refresh = false)
	{
		...
		$hook_query_fields_ids = $hook_query_joins_ids = $hook_query_where_ids = '';
		...
		// Grab all ids that belong to this item to filter out deleted / moderated -- if we can see deleted or moderated items that is!
		$idlist = array(0);
		if (!$this->no_limit AND $this->parent_id AND !$this->filter_id AND ($this->show_deleted OR $this->show_moderated))
		{
			$this->calctotal = false;
			$this->total = 0;
			$ids = $this->registry->db->query_read_slave("
				SELECT gm.gmid, gm.state
					$hook_query_fields_ids
				FROM " . TABLE_PREFIX . "groupmessage AS gm
				$hook_query_joins_ids
				WHERE
					$hook_query_where_ids
					gm.discussionid = {$this->parent_id}
				" . ($this->daysprune ? (($this->daysprune != -1) ? " AND gm.dateline >= " . (TIMENOW - ($this->daysprune * 86400)) : '') : '') . "
				" . "ORDER BY " . $this->sortfield . " " . ($this->descending ? "DESC" : "ASC")
			);
			$limitlower = ($this->page - 1) * $this->quantity;
			$limitupper = $this->page * $this->quantity;
			if ($limitlower != 0)
			{
				$limitlower++;
			}
			while ($item = $this->registry->db->fetch_array($ids))
			{
				if ($item['state'] == 'visible')
				{
					$this->total++;
				}
				if ($this->total < $limitlower OR $this->total > $limitupper)
				{
					continue;
				}
			...
			$sql .= " AND gm.gmid IN (" . implode(',', $idlist) . ")";
			$no_limit = true;
		}
		...
		$sql .= 	" "	. $this->state_sql() .
					...
					(($no_limit OR $this->no_limit) ? "" : "LIMIT {$this->start}, {$this->quantity}"));
...
class vB_Group_Collection_Discussion extends vB_Group_Collection_Message
{
	...
	function collection_sql($force_refresh = false)
	{
		...
		if (!$this->show_unsubscribed)
		{
			$sqlout = " INNER JOIN " . TABLE_PREFIX . "subscribediscussion AS subscribediscussion
		...
		if (!$this->show_read)
		{
			...
			$sqlout = " AND d.lastpost > $oldtime AND d.lastpost > COALESCE(groupread.readtime, 0) AND d.lastpost > COALESCE(discussionread.readtime, 0)";
			$sql .= $sqlout;
			$idwhere .= $sqlout;
		}
		...
		{
			$sqlout = " AND (MATCH(gm.title, gm.pagetext) AGAINST ('" . $this->registry->db->escape_string($this->filter_searchtext) . "' IN BOOLEAN MODE) )";
			$sql .= $sqlout;
			$idwhere .= $sqlout;
		}
		...
		if ($this->filter_searchtext AND 'relevance' == $this->sortfield)
		{
			$relevancesql = " ,(MATCH(gm.title, gm.pagetext) AGAINST ('" . $this->registry->db->escape_string($this->filter_searchtext) . "' IN BOOLEAN MODE) ) AS relevance";
		}
		else
		{
			$relevancesql = "";
		...
		if (!$this->no_limit AND $this->parent_id AND !$this->filter_id AND ($this->show_deleted OR $this->show_moderated))
		{
			...
			$ids = $this->registry->db->query_read_slave("
				...
				$hook_query_joins_ids
				WHERE
					d.groupid = {$this->parent_id}
				" . ($this->daysprune ? (($this->daysprune != -1) ? " AND d.lastpost >= " . (TIMENOW - ($this->daysprune * 86400)) : '') : '') . "
				$idwhere
				$hook_query_where_ids
				" . "ORDER BY " . $this->sortfield . " " . ($this->descending ? "DESC" : "ASC")
			);
			...
			while ($item = $this->registry->db->fetch_array($ids))
			{
				...
				$idlist[] = $item['discussionid'];
			}
			$sql .= " AND d.discussionid IN (" . implode(',', $idlist) . ")";
			$no_limit = true;
		}
...
class vB_Bit_Factory
{
	...
	* @param	int		dummy for php 5.4 compatability, quick fix as we are not rewriting the code
	* @return	vB_Bit
	*/
	function &create($item, $group = 0)
	{
...
class vB_Bit
{
	...
	*/
	function __construct(&$registry, &$factory, $item)
	{
	...
	*/
	function process_registered_user()
	{
		...
		$show['pmlink'] = ($this->registry->options['enablepms'] AND $this->registry->userinfo['permissions']['pmquota'] AND ($this->registry->userinfo['permissions']['pmpermissions'] & $this->registry->bf_ugp_pmpermissions['canignorequota']
...
class vB_Group_Bit extends vB_Bit
{
	...
	*/
	function __construct(&$registry, &$factory, $item, $group)
	{
		parent::__construct($registry, $factory, $item);
		$this->group = $group;
	}
...
class vB_Group_Bit_Message extends vB_Group_Bit
{
	...
	*/
	function prepare_end()
	{
		...
		{
			$this->item['itemipaddress'] = ($this->item['itemipaddress'] ? $this->item['itemipaddress'] : '');
		}
		else
		{
