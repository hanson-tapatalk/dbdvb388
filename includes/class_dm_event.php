	...
	function pre_save($doquery = true)
	{
		...
		if ($this->verify_datetime)
		{
			...
			if ($this->info['type'] != 'single')
			{
				...
				require_once(DIR . '/includes/functions_misc.php');
				$this->set_info('occurdate', vbgmdate('Y-n-j', $dateline_from + intval($this->registry->userinfo['timezoneoffset']) * 3600, false, false));
			}
			...
			if ($this->info['type'] == 'recur')
			{
				$checkevent = array(
					...
					'dateline_from_user' => $dateline_from + intval($this->registry->userinfo['timezoneoffset']) * 3600,
					'dateline_to_user'   => $dateline_to + intval($this->registry->userinfo['timezoneoffset']) * 3600,
