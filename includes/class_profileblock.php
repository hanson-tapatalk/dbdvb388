...
class vB_ProfileBlockFactory
{
	...
	*/
	function __construct(&$registry, &$profile)
	{
...
class vB_ProfileBlock
{
	...
	*/
	function __construct(&$registry, &$profile, &$factory)
	{
		$this->registry =& $registry;
		$this->profile =& $profile;
	...
	function prepare_output($id = '', $options = array())
	{
		...
		foreach ($imservices AS $imserviceid => $iminfo)
		{
			if ($this->profile->prepared["$imserviceid"])
			{
				...
				($hook = vBulletinHook::fetch_hook('member_profileblock_contactinfobit')) ? eval($hook) : false;
