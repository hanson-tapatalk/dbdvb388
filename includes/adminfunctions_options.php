...
function print_setting_row($setting, $settingphrase)
{
	...
	switch ($setting['optioncode'])
	{
		...
		case 'usergroup':
		{
			...
			if ($size > 1)
			{
				print_select_row($description, $name . '[]', array(0 => '') + $usergrouplist, vb_unserialize($setting['value']), false, $size, true);
			}
			else
			{
		...
		case 'multiinput':
		{
			...
			$setting['values'] = vb_unserialize($setting['value']);
...
function xml_import_settings($xml = false)
{
	...
	foreach($arr['settinggroup'] AS $group)
	{
		...
		$qBits = array();
		if (empty($group['setting'][0]) OR !is_array($group['setting'][0]))
		{
		...
		foreach($group['setting'] AS $setting)
		{
			...
			{
				$newvalue = empty($setting['defaultvalue']) ? NULL : $setting['defaultvalue'];
			}
			$qBits[] = "(
				'" . (empty($setting['varname']) ? '' : $vbulletin->db->escape_string($setting['varname'])) . "',
				'" . (empty($group['name']) ? '' : $vbulletin->db->escape_string($group['name'])) . "',
				'" . (empty($newvalue) ? '' : $vbulletin->db->escape_string(trim($newvalue))) . "',
				'" . (empty($setting['defaultvalue']) ? '' : $vbulletin->db->escape_string(trim($setting['defaultvalue']))) . "',
				'" . (empty($setting['datatype']) ? '' : $vbulletin->db->escape_string(trim($setting['datatype']))) . "',
				'" . (empty($setting['optioncode']) ? '' : $vbulletin->db->escape_string($setting['optioncode'])) . "',
				" . (empty($setting['displayorder']) ? 0 : intval($setting['displayorder'])) . ",
				" . (empty($setting['advanced']) ? 0 : intval($setting['advanced'])) . ",
				1" . (!defined('UPGRADE_COMPAT') ? ",
					'" . (empty($setting['validationcode']) ? '' : $vbulletin->db->escape_string($setting['validationcode'])) . "',
					" . (empty($setting['blacklist']) ? 0 : intval($setting['blacklist'])) . ",
					'" . (empty($product) ? '' : $vbulletin->db->escape_string($product)) . "'" : '') . "\n\t)";
		}
