	...
	*/
	function start($toemail, $subject, $message, $from = '', $uheaders = '', $username = '')
	{
		...
		if (empty($from))
		{
			...
			// Exchange (Oh Microsoft) doesn't respect auto-generated
	...
	function encode_email_header($text, $charset = 'utf-8', $force_encode = false, $quoted_string = true)
	{
		...
		if ($qp_encode == true)
		{
			...
			$outtext = preg_replace_callback(
				...
				function($matches)
				{
					return '='.strtoupper(dechex(ord($matches[1]))); // The '=' is intentional and required
				},
				$text);
	...
	*/
	function log_email($status = true)
	{
		if (!empty($this->registry->options['errorlogemail']) AND !is_demo_mode())
		{
			...
			$destination = "$errfile.log";
			$fp = @fopen($destination, 'a+b');
...
class vB_SmtpMail extends vB_Mail
{
	...
	*/
	function send()
	{
		...
		$trigger_error = $send_mail = true;
		$stream = ($this->secure == 'ssl' ? 'ssl://' : 'tcp://');
		$stream = ($this->secure == 'tlsn' ? 'tls://' : $stream);
		
		($hook = vBulletinHook::fetch_hook('mail_smtp_send_before')) ? eval($hook) : false;
		...
		if ($this->smtpSocket)
		{
			...
			if (($this->secure == 'tls' OR $this->secure == 'tlsn') AND function_exists('stream_socket_enable_crypto'))
			{
		...
		($hook = vBulletinHook::fetch_hook('mail_smtp_send_after')) ? eval($hook) : false;
	}
}
...
class vB_QueueMail extends vB_Mail
{
	...
	*/
	public static function fetch_instance()
	{
		...
		{
			global $vbulletin;
			self::$instance = new vB_QueueMail($vbulletin);
		}
	...
	*/
	function exec_queue()
	{
		...
		if (!empty($mailqueueids))
		{
			...
			{
				// send those mails
				$mail = clone $prototype; // avoid ctor overhead
