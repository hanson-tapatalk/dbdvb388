...
class vB_DataManager
{
	...
	*/
	function set($fieldname, $value, $clean = true, $doverify = true)
	{
		if ($clean)
		{
			...
			{
				if ($this->validfields["$fieldname"][VF_REQ] AND sizeof($this->errors) < 1)
				{
	...
	* By default this does nothing, just returns true
	* Override in each child DM as needed to store the IP.
	*
	...
	*/
	function save_ipaddress($type = '', $fieldid = '')
	{
		return true;
	}
	...
	*/
	...
	/**
	* Verifies an IP address - IPv4 or IPv6
	*
	...
	*/
	function verify_ipaddress($ipaddress)
	{
		...
		{
			return (get_iptype($ipaddress) ? true : false);
		}
	}
	...
	* @return	boolean
	*/
	public function verify_md5($md5)
	{
		return self::verifyMd5($md5);
	}

	public static function verifyMd5($md5)
	{
