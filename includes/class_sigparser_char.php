...
class vB_SignatureParser_CharCount extends vB_SignatureParser
{
	...
	/**
	 * @var int Computed ratio for calculating size. Set by handle_bbcode_size and used by handle_bbcode_size_callback.
	 */
	protected $computed_ratio = 0;
	...
	*/
	function __construct(&$registry, $tag_list, $usergroup_signature_permission, $userid, $append_custom_tags = true)
	{
		parent::__construct($registry, $tag_list, $usergroup_signature_permission, $userid, $append_custom_tags);
	...
	*/
	...
	function handle_bbcode_size($value, $size)
	{
		...
		else
		{
			...
			// by the parent size ratio to account for nested size tags, by increasing
			// the size of the output if there parent size is < 2, or decreasing if > 2
			$this->computed_ratio = $this->size_ratios["$size"] / $this->size_ratios["$parent_size"];
		}

		return preg_replace_callback('#([^\n]+)#', array($this, 'handle_bbcode_size_callback'), $value);
	}

	/**
	 * Callback for the size bbcode
	 *
	 * @param	array	RE matches. Array indeces used:
	 * 			1 - The size value
	 *
	 * @return	string	The calculated size value
	 */
	function handle_bbcode_size_callback($matches)
	{
		return str_repeat('*', ceil(strlen($matches[1]) * $this->computed_ratio));
	}
	...
	*/
	function handle_bbcode_img($bbcode, $do_imgcode, $has_img_code = false)
	{
		...
		if ($has_img_code & BBCODE_HAS_IMG)
		{
			...
			{
				$bbcode = preg_replace('#\[img\]\s*(https?://([^*\r\n]+|[a-z0-9/\\._\- !]+))\[/img\]#iU', '$1', $bbcode);
			}
		}

		if ($has_img_code & BBCODE_HAS_SIGPIC)
		{
			$bbcode = preg_replace('#\[sigpic\](.*)\[/sigpic\]#siU', '', $bbcode);
		}
