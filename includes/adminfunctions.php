...
function print_cp_login($mismatch = false)
{
	...
	?>
	<script type="text/javascript" src="../clientscript/vbulletin_md5.js?v=<?php echo $vbulletin->options['simpleversion'];?>"></script>
...
function print_cp_header($title = '', $onload = '', $headinsert = '', $marginwidth = 0, $bodyattributes = '')
{
	...
	if (!empty($vbulletin->options['gzipoutput']) AND !$vbulletin->nozip AND !headers_sent() AND function_exists('ob_start') AND function_exists('crc32') AND function_exists('gzcompress'))
	{
	...
	switch(VB_AREA)
	{
		...
		case 'Install': $titlestring = iif($title, "vBulletin $title - ") . (!empty($vbulletin->options['bbtitle']) ?  $vbulletin->options['bbtitle'] : ''); break;
	...
	<script type=\"text/javascript\">
	<!--
	var SESSIONHASH = \"" . (empty($vbulletin->session->vars['sessionhash']) ? '' : $vbulletin->session->vars['sessionhash']) . "\";
	...
	<script type=\"text/javascript\" src=\"../clientscript/yui/yahoo-dom-event/yahoo-dom-event.js?v=" . $vbulletin->options['simpleversion'] . "\"></script>
	<script type=\"text/javascript\" src=\"../clientscript/yui/connection/connection-min.js?v=" . $vbulletin->options['simpleversion'] . "\"></script>
	<script type=\"text/javascript\" src=\"../clientscript/vbulletin_global.js?v=" . $vbulletin->options['simpleversion'] . "\"></script>\n\r";
	echo "</head>\r\n";
...
function print_cp_footer()
{
	...
	if (!empty($vbulletin->debug))
	{
		...
		$_REQUEST['do'] = empty($_REQUEST['do']) ? '' : htmlspecialchars_uni($_REQUEST['do']);
	...
	if (!empty($vbulletin->options['gzipoutput']) AND function_exists("ob_start") AND function_exists("crc32") AND function_exists("gzcompress") AND !$vbulletin->nozip)
	{
...
function print_submit_row($submitname = '', $resetname = '_default_', $colspan = 2, $goback = '', $extra = '', $alt = false)
{
	...
	// do debug tooltip
	if ($vbulletin->debug AND isset($GLOBALS['_HIDDENFIELDS']) AND is_array($GLOBALS['_HIDDENFIELDS']))
	{
...
function print_yes_no_row($title, $name, $value = 1, $onclick = '')
{
	...
	require_once(DIR . '/includes/functions_misc.php');
	$yes_but_not_parsing_html = fetch_phrase('yes_but_not_parsing_html', 'cpuser');
	...
	print_label_row(
		...
		iif($value == 2 AND $name == 'customtitle', "
			<label for=\"rb_2_{$name}_$uniqueid\"><input type=\"radio\" name=\"$name\" id=\"rb_2_{$name}_$uniqueid\" value=\"2\" tabindex=\"1\"$onclick" . iif($vbulletin->debug, " title=\"name=&quot;$name&quot; value=&quot;2&quot;\"") . " checked=\"checked\" />{$yes_but_not_parsing_html}</label>"
		) . "\n\t</div>",
		'', 'top', $name
	);
}
...
function print_time_row($title, $name = 'date', $unixtime = '', $showtime = true, $birthday = false, $valign = 'middle')
{
	...
	{
		echo '
			<script type="text/javascript" src="../clientscript/vbulletin_date_picker.js?v=' . $vbulletin->options['simpleversion'] . '"></script>
			<script type="text/javascript">
			<!--
...
function fetch_forum_order($parentid = -1, $depth = 0)
{
	...
	if (!empty($vbulletin->iforumcache["$parentid"]) AND is_array($vbulletin->iforumcache["$parentid"]))
	{
...
/**
* Verifies that the optimizer you are using with vB is compatible. 
* Bugs in various versions of optimizers such as Turck MMCache and eAccelerator
...
{
	// fail if eAccelerator or Turck is loaded
	...
	{
		return 'eaccelerator_not_supported';
	}
	else if (extension_loaded('apcu'))
	{
		if (version_compare(PHP_VERSION, '7.0.0', '>='))
		{
			// PHP 7.x
			$reqver = '5.1.5';
			$errmsg = 'apcu_too_old_7'; 
		}
		else
		{
			// PHP 5.x
			$reqver = '4.0.11';
			$errmsg = 'apcu_too_old_5';
		}
		// first, attempt to use phpversion()...
		if ($apcu_version = phpversion('apcu'))
		{
			if (version_compare($apcu_version, $reqver, '<'))
			{
				return $errmsg;
			}
		}
		...
		else if (function_exists('phpinfo') AND function_exists('ob_start') AND @ob_start())
		{
			...
			@ob_end_clean();
			preg_match('#<tr class="h"><th>APCu support</th><th>enabled</th></tr>(?:\s+)<tr><td class="e">Version </td><td class="v">(.*?)</td></tr>#si', $info, $hits);
			if (!empty($hits[0]))
			{
				$apcu_version = trim($hits[1]);
