...
function fetch_online_status(&$user, $setstatusimage = false)
{
	...
	if ($user['lastactivity'] > $datecut AND (!isset($user['lastvisit']) OR $user['lastvisit'] != $user['lastactivity']))
	{
		if ($user['invisible'])
		{
...
function construct_forum_rules($foruminfo, $permissions)
{
	...
	$bbcodeon = !empty($foruminfo['allowbbcode']) ? $vbphrase['on'] : $vbphrase['off'];
	$imgcodeon = !empty($foruminfo['allowimages']) ? $vbphrase['on'] : $vbphrase['off'];
	$htmlcodeon = !empty($foruminfo['allowhtml']) ? $vbphrase['on'] : $vbphrase['off'];
	$smilieson = !empty($foruminfo['allowsmilies']) ? $vbphrase['on'] : $vbphrase['off'];
