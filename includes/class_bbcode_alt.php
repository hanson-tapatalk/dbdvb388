...
class vB_BbCodeParser_Wysiwyg extends vB_BbCodeParser
{
	...
	*/
	...
	protected function strip_smilies_callback($matches)
	{
		return $this->strip_smilies($matches[0]);
	}
...
class vB_BbCodeParser_PlainText extends vB_BbCodeParser
{
	...
	function append_custom_tags()
	{
		...
		else // query bbcodes out of the database
		{
			...
			while ($customtag = $this->registry->db->fetch_array($bbcodes))
			{
				...
				$this->tag_list["$has_option"]["$customtag[bbcodetag]"] = array(
					'html' => '%1$s',
					'strip_empty'		=> empty($customtag['options']) ? 0 : (intval($customtag['options']) & $this->registry->bf_misc['bbcodeoptions']['strip_empty']),
					'stop_parse' 		=> empty($customtag['options']) ? 0 : (intval($customtag['options']) & $this->registry->bf_misc['bbcodeoptions']['stop_parse']),
					'disable_smilies'	=> empty($customtag['options']) ? 0 : (intval($customtag['options']) & $this->registry->bf_misc['bbcodeoptions']['disable_smilies']),
					'disable_wordwrap'	=> empty($customtag['options']) ? 0 : (intval($customtag['options']) & $this->registry->bf_misc['bbcodeoptions']['disable_wordwrap'])
				);
			}
		}
	}
	...
	*/
	function handle_bbcode_img($bbcode, $do_imgcode, $has_img_code = false)
	{
		...
		{
			// do [img]xxx[/img]
			$bbcode = preg_replace_callback('#\[img\]\s*(https?://([^<>*"]+|[a-z0-9/\\._\- !]+))\[/img\]#iU', array($this, 'bbcode_img_match_callback'), $bbcode);
		}
