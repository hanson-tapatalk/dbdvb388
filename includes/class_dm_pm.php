...
class vB_DataManager_PM extends vB_DataManager
{
	...
	*/
	function verify_touserarray(&$tousers)
	{
		...
		if (!is_array($tousers))
		{
			$tousers = vb_unserialize($tousers);
	...
	*/
	function set_recipients($recipientlist, &$permissions, $type = 'bcc')
	{
		...
		if (!empty($this->info['is_automated']) OR $permissions['pmpermissions'] & $this->registry->bf_ugp_pmpermissions['canignorequota'])
		{
		...
		foreach ($users AS $lowname => $user)
		{
			if (!($user['options'] & $this->registry->bf_misc_useroptions['receivepm']) AND !$this->overridequota)
			{
				...
				($user['options'] & $this->registry->bf_misc_useroptions['receivepmbuddies'])
					AND
				strpos(" $user[buddylist] ", " $fromuser[userid] ") === false
					AND
				!can_moderate()
					AND
				!$this->overridequota
			...
			else
			{
				...
				else if (strpos(" $user[ignorelist] ", ' ' . $fromuser['userid'] . ' ') !== false AND !$this->overridequota)
				{
					...
					{
						// this bit will never be reached
