...
function fetch_visitormessageinfo($vmid)
{
	...
	if (!isset($messagecache["$vmid"]))
	{
		$messagecache["$vmid"] = $vbulletin->db->query_first("
			SELECT *, ipaddress.ip AS ipaddress
			FROM " . TABLE_PREFIX . "visitormessage AS visitormessage
			LEFT JOIN " . TABLE_PREFIX . "ipaddress AS ipaddress ON (ipaddress.ipid = visitormessage.ipaddress)
...
function fetch_vm_ajax_query($userinfo, $vmid, $type = 'wall', $userinfo2 = null)
{
	...
	if ($type != 'wall')
	{
		...
		$sql = "
			SELECT
				visitormessage.*, user.*, ipaddress.ip AS messageipaddress, visitormessage.userid AS profileuserid, ipaddress.ip AS ipaddress
	...
	else
	{
		...
		$sql = "
			(
				SELECT
					visitormessage.*, visitormessage.dateline AS pmdateline, user.*, ipaddress.ip AS messageipaddress, visitormessage.userid AS profileuserid, ipaddress.ip AS ipaddress
					$delsql1
			...
			(
				SELECT
					visitormessage.*, visitormessage.dateline AS pmdateline, user.*, ipaddress.ip, visitormessage.userid AS profileuserid, ipaddress.ip AS ipaddress
