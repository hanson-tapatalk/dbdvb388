...
function construct_faq_item($faq, $find = '')
{
	...
	{
		$faq['title'] = preg_replace_callback('#(^|>)([^<]+)(?=<|$)#sU', function($matches) use ($find)
		{
			return process_highlight_faq($matches[2], $find, $matches[1], '<u>$1</u>');

		}, $faq['title']);

		$faq['text'] = preg_replace_callback('#(^|>)([^<]+)(?=<|$)#sU', function($matches) use ($find)
		{
			return process_highlight_faq($matches[2], $find, $matches[1], '<span class="highlight">$1</span>');
