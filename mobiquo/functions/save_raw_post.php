...
function save_raw_post_func($xmlrpc_params){
    ...
    $_POST['reason'] = mobiquo_encode($_POST['reason'], 'to_local');
	...
	$vbulletin->input->clean($posttitle, TYPE_NOHTML);
	$postcontent = in_text_clean(mobiquo_encode($decode_params[2],'to_local'));
	$html_content= false;
	...
	$edit['parseurl'] = (($vbulletin->options['allowedbbcodes'] & ALLOW_BBCODE_URL) AND $foruminfo['allowbbcode']);
	$edit['signature'] = $GLOBALS['tt_config']['forum_signature'];
	...
	if(isset($decode_params[5]) && !empty($decode_params[5]))
		$posthash = $decode_params[5];

	//correct the userid of attachment attached just now
    if($posthash && $postinfo['userid']){
        $db->query_write("UPDATE " . TABLE_PREFIX . "attachment SET userid = ".$db->escape_string($postinfo['userid'])." WHERE
...
//			}
	...
	$new_post = get_post_from_id($postinfo[postid], $html_content);
