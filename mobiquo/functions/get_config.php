...
function get_config_func()
{
    ...
    $return_config = array(
        ...
        'php_version'   => new xmlrpcval(phpversion(), 'string'),
    );
    ...
    if ($isTTServerCall)
    {
        $return_config['release_timestamp'] = new xmlrpcval($tt_config['release_timestamp'],'string');
        $return_config['smartbanner_info'] = new xmlrpcval(json_encode(isset($vbulletin->options['tapatalk_banner_data']) ? unserialize($vbulletin->options['tapatalk_banner_data']) :  array('banner_enable' => -1)), 'string');
        ...
        if(isset($ex[0]))
        {
            ...
            $return_config['push_slug'] = new xmlrpcval(json_encode($push_slug), 'string');
        }
    ...
    foreach($tt_config as $key => $value)
    {
        ...
        }else if((!isset($return_config[$key]) || !$return_config[$key]) && !is_array($value) && !in_array($key, array('sys_version'))) {
    ...
    if ($isTTServerCall){
        ...
        $return_config['set_api_key'] = new xmlrpcval(1, 'string');
        $return_config['user_subscription'] = new xmlrpcval(1, 'string');
        $return_config['push_content_check'] = new xmlrpcval(1, 'string');
        $return_config['php_version'] = new xmlrpcval(phpversion(), 'string');
    ...
    if (!isset($vbulletin->options['TT_tapatalk_banner_control']['banner_control']))
        $return_config['banner_control'] = new xmlrpcval(1, 'string');
    else 
        $return_config['banner_control'] = new xmlrpcval($vbulletin->options['TT_tapatalk_banner_control']['banner_control'], 'string');
    
    if(isset($vbulletin->options['post_thanks_on_off']) && $vbulletin->options['post_thanks_on_off'] == 1) 
    {
        $return_config['push_type'] = new xmlrpcval('quote,tag,newtopic,pm,sub,thank,newsub', 'string');
    }
    ...
    {
        $return_config['push_type'] = new xmlrpcval('quote,tag,newtopic,pm,sub,thank,like,newsub,deltopic,delpost', 'string');
    }
