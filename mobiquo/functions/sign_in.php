...
class TTSSOForum implements TTSSOForumInterface
{
    ...
    public function createUserHandle($email, $username, $password, $verified, $custom_register_fields, $profile, &$errors)
    {
        ...
        if(!empty($custom_fields))
        {
            ...
            {
                //hv_question verify
                if(preg_match('/hv_input,.*/', $filed_name))
                {
                    $arr = explode(",", $filed_name);
                    $hv_verify['input'] = $field_value;
                    $hv_verify['hash'] = $arr[1];
                    ...
                    if (fetch_require_hvcheck('register'))
                    {
                        ...
                        if (!$tt_verify->verify_token($hv_verify))
                        {
                            $errors[] = 'The answer given for the random question was incorrect.';
                            return false;
                        }
                    }
                }
                if($filed_name == "COPPA_BIRTHDAY" || $filed_name == "REQ_BIRTHDAY")
                {
        ...
        else
        {
            ...
            $result_text = mobiquo_encode($result_text);
            $errors[] = $result_text;
            return false;//error_status($username_exist ? '1' : '0', $result_text);
        }
    }
