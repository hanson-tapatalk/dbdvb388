...
function get_thread_content($threadid, $start_num, $end_num, $position = 0)
{
    ...
    //for dbtech thanks/like
    if((isset($vbulletin->options['dbtech_thanks_features']) && $vbulletin->options['dbtech_thanks_features'] && isset($vbulletin->products['dbtech_thanks']) && $vbulletin->products['dbtech_thanks']))
    {
        require(DIR . '/dbtech/thanks/hooks/showthread_query.php');
    }
    ...
    while ($post = $db->fetch_array($posts))
    {
        ...
        if ( $fetchtype == 'post' or $fetchtype == 'post_ignore' OR ($fetchtype == 'post_deleted' AND $show['viewpost']))
        {
            ...
            if(is_array($mobiquo_attachments)){
                foreach($mobiquo_attachments as $attach) {
                    ...
                    $return_attachment = new xmlrpcval(array(
                        ...
                        'can_view_url' => new xmlrpcval($viewpermission != 0, 'boolean'),
                        'can_view_thumbnail_url' => new xmlrpcval($viewthumbpermission, 'boolean'),
                  ), 'struct');
            ...
            if(is_array($mobiquo_inlineattachments)){
                foreach($mobiquo_inlineattachments as $attach) {
                    ...
                    if(empty($attachment_url)){
                        $attachment_url = $GLOBALS['vbulletin']->options['bburl'] . "/attachment.php?" . $GLOBALS['vbulletin']->session->vars['sessionurl_js'] . "attachmentid=". $attach['attachmentid']. "&d=" . $attach[dateline];
                    }
            ...
            $post_content = preg_replace_callback('/\[tp_noparse\](.*?)\[\/tp_noparse\]/si', "callback_base64_decode", $post_content);

            // add spoiler for user ignored post
            if ($fetchtype == 'post_ignore')
            {
                $post_content = "[spoiler]{$post_content}[/spoiler]";
            }
            ...
            $return_post = array(
                ...
                'post_author_user_type' => new xmlrpcval(get_usertype_by_item($post['userid'],$post['usergroupid']), 'string'),
            ...
            // Add dbTech Support
            if(isset($vbulletin->options['dbtech_thanks_features']) && $vbulletin->options['dbtech_thanks_features'] && isset($vbulletin->products['dbtech_thanks']) && $vbulletin->products['dbtech_thanks'] && class_exists('THANKS'))
            {
                require_once('dbtTL.php');
                if($vbulletin->options['dbtech_thanks_features'] == 1 || $vbulletin->options['dbtech_thanks_features'] > 2)
                {
                    $return_post = dbTTL($return_post, $post, $thread['forumid'], 'thanks', 'can_thank', '', '', 'thanks_info');
                }
                if($vbulletin->options['dbtech_thanks_features'] == 2 || $vbulletin->options['dbtech_thanks_features'] > 2)
                {
                    $return_post = dbTTL($return_post, $post, $thread['forumid'], 'likes', 'can_like', 'is_liked', 'like_count', 'likes_info');
                }
            }
            // vbseo like support
    ...
    if (!$threadinfo['open'] && !can_moderate($threadinfo['forumid'], 'canopenclose'))
    {
        $mobiquo_can_reply = false;
    }
    ...
    $return_data = array(
        ...
        'topic_author_user_type' => new xmlrpcval(get_usertype_by_item($thread['postuserid'],$thread['postusergroupid']), 'string'),
    ...
    if ($mobiquo_can_rename)            $return_data['can_rename']      = new xmlrpcval(true, 'boolean');
    ...
    if ($thread['prefixid'])            $return_data['prefix_id']       = new xmlrpcval($thread['prefixid'], 'string');
     ...
     //new added logic, if not moderator, not return can_merge and can_stick
    if( !can_moderate($thread['forumid']) )
    {
        $return_data['can_stick']       = new xmlrpcval(false, 'boolean');
        $return_data['can_merge']       = new xmlrpcval(false, 'boolean');
    }
