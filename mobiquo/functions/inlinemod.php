...
if ($_POST['do'] == 'open' OR $_POST['do'] == 'close')
{
	...
	while ($thread = $db->fetch_array($threads))
	{
		...
		//add access for topic creater if it's setted up
        $can_openclose = (can_moderate($thread['forumid'], 'canopenclose') OR ($forumperms & $vbulletin->bf_ugp_forumpermissions['canopenclose'] AND $thread['postuserid'] == $vbulletin->userinfo['userid'])) ? true : false;
        if (!$can_openclose)
        {
...
if ($_POST['do'] == 'deletethread' OR $_POST['do'] == 'spamthread')
{
	...
	while ($thread = $db->fetch_array($threads))
	{
		...
		else if (!can_moderate($thread['forumid'], 'canremoveposts'))
		{
			...
			{
				return_mod_fault(fetch_error('you_do_not_share_delete_permission'));
			}
			else
			{
...
if ($_POST['do'] == 'spamconfirm' OR $_POST['do'] == 'dodeletespam')
{
    ...
    $userid_from_name = get_userid_by_name($username);

    if(!$userid_from_name)
        return_mod_fault('The user you want to ban does not exist!');

    $_POST['userid'] = $vbulletin->GPC['userid'] = array($userid_from_name);
/*	$userids = array();
	...
	{ // posts
		...
		while ($post = $db->fetch_array($posts))
		{
			...
			else if (!can_moderate($post['forumid'], 'canremoveposts'))
			{
				...
				{
					return_mod_fault(fetch_error('you_do_not_have_permission_to_delete_threads_and_posts', $post['title'], $post['thread_title'], $vbulletin->forumcache["$post[forumid]"]['title']));
				}
			}
	...
	if (!empty($user_cache))
	{
		switch ($vbulletin->GPC['useraction'])
		{
			case 'ban':
				...
				$vbulletin->GPC['usergroupid'] = 8;//specify to ban group
				...
				{
				    //specify random ban group if default ban group id 8 has been changed
				    if(!empty($vbulletin->usergroupcache) && is_array($vbulletin->usergroupcache))
				        foreach($vbulletin->usergroupcache as $groupid => $usergroup)
				            if(!($vbulletin->usergroupcache["{$groupid}"]['genericoptions'] & $vbulletin->bf_ugp_genericoptions['isnotbannedgroup']))
				...
				if ($vbulletin->GPC['period'] == 'PERMANENT' OR $vbulletin->GPC['period'] == 0)
				{
				...
				else
				{
					...
					$timestamp = $vbulletin->GPC['period'];

					$liftdate1 = convert_date_to_timestamp(D_1)-300;
										$liftdate2 = convert_date_to_timestamp(D_2)-300;
										$liftdate3 = convert_date_to_timestamp(D_3)-300;
										$liftdate4 = convert_date_to_timestamp(D_4)-300;
										$liftdate5 = convert_date_to_timestamp(D_5)-300;
										$liftdate6 = convert_date_to_timestamp(D_6)-300;
										$liftdate7 = convert_date_to_timestamp(D_7)-300;
										$liftdate8 = convert_date_to_timestamp(D_10)-300;
										$liftdate9 = convert_date_to_timestamp(D_14)-300;
										$liftdate10 = convert_date_to_timestamp(D_21)-300;
										$liftdate11 = convert_date_to_timestamp(M_1)-300;
										$liftdate12 = convert_date_to_timestamp(M_2)-300;
										$liftdate13 = convert_date_to_timestamp(M_3)-300;
										$liftdate14 = convert_date_to_timestamp(M_4)-300;
										$liftdate15 = convert_date_to_timestamp(M_6)-300;
										$liftdate16 = convert_date_to_timestamp(Y_1)-300;
										$liftdate17 = convert_date_to_timestamp(Y_2)-300;

										if ($liftdate1 <= $timestamp & $liftdate2 > $timestamp)
										{
											$liftdate = $liftdate1;
										}
										if ($liftdate2 <= $timestamp & $liftdate3 > $timestamp)
										{
											$liftdate = $liftdate2;
										}
										if ($liftdate3 <= $timestamp & $liftdate4 > $timestamp)
										{
											$liftdate = $liftdate3;
										}
										if ($liftdate4 <= $timestamp & $liftdate5 > $timestamp)
										{
											$liftdate = $liftdate4;
										}
										if ($liftdate5 <= $timestamp & $liftdate6 > $timestamp)
										{
											$liftdate = $liftdate5;
										}
										if ($liftdate6 <= $timestamp & $liftdate7 > $timestamp)
										{
											$liftdate = $liftdate6;
										}
										if ($liftdate7 <= $timestamp & $liftdate8 > $timestamp)
										{
											$liftdate = $liftdate7;
										}
										if ($liftdate8 <= $timestamp & $liftdate9 > $timestamp)
										{
											$liftdate = $liftdate8;
										}
										if ($liftdate9 <= $timestamp & $liftdate10 > $timestamp)
										{
											$liftdate = $liftdate9;
										}
										if ($liftdate10 <= $timestamp & $liftdate11 > $timestamp)
										{
											$liftdate = $liftdate10;
										}
										if ($liftdate11 <= $timestamp & $liftdate12 > $timestamp)
										{
											$liftdate = $liftdate11;
										}
										if ($liftdate12 <= $timestamp & $liftdate13 > $timestamp)
										{
											$liftdate = $liftdate12;
										}
										if ($liftdate13 <= $timestamp & $liftdate14 > $timestamp)
										{
											$liftdate = $liftdate13;
										}
										if ($liftdate14 <= $timestamp & $liftdate15 > $timestamp)
										{
											$liftdate = $liftdate14;
										}
										if ($liftdate15 <= $timestamp & $liftdate16 > $timestamp)
										{
											$liftdate = $liftdate15;
										}
										if ($liftdate16 <= $timestamp & $liftdate17 > $timestamp)
										{
											$liftdate = $liftdate16;
										}
										if ($liftdate17 <= $timestamp)
										{
											$liftdate = $liftdate17;
										}
				}
...
if ($_POST['do'] == 'dodeletethreads')
{
	...
	while ($thread = $db->fetch_array($threads))
	{
		...
		{
			//add access for topic creater if is setted up
            $threadowner_candelete = ($forumperms & $vbulletin->bf_ugp_forumpermissions['candeletepost'] AND $forumperms & $vbulletin->bf_ugp_forumpermissions['candeletethread'] AND $vbulletin->userinfo['userid'] == $thread['postuserid'] AND ($vbulletin->options['edittimelimit'] == 0 OR $thread['dateline'] > (TIMENOW - ($vbulletin->options['edittimelimit'] * 60)))) ? true : false;

			if (!can_moderate($thread['forumid'], 'canremoveposts') AND $physicaldel)
			{
				return_mod_fault(fetch_error('you_do_not_have_permission_to_delete_threads_and_posts', $vbphrase['n_a'], $thread['prefix_plain_html'] . $thread['title'], $vbulletin->forumcache["$thread[forumid]"]['title']));
			}
			else if (!can_moderate($thread['forumid'], 'candeleteposts') AND !$physicaldel AND !$threadowner_candelete)
			{
				return_mod_fault(fetch_error('you_do_not_have_permission_to_delete_threads_and_posts', $vbphrase['n_a'], $thread['prefix_plain_html'] . $thread['title'], $vbulletin->forumcache["$thread[forumid]"]['title']));
			}
		}
...
if ($_POST['do'] == 'domovethreads')
{
	...
	while ($thread = $db->fetch_array($threads))
	{
		...
		//add access for topic creater if is setted up
        $can_move = (can_moderate($thread['forumid'], 'canmanagethreads') OR ($forumperms & $vbulletin->bf_ugp_forumpermissions['canmove'] AND $thread['postuserid'] == $vbulletin->userinfo['userid'])) ? true : false;
        if (!$can_move)
	{
...
if ($_POST['do'] == 'approveattachments' OR $_POST['do'] == 'unapproveattachments')
{
	...
	while ($post = $db->fetch_array($posts))
	{
		...
		{
			return_mod_fault(fetch_error('you_do_not_have_permission_to_manage_moderated_attachments', $post['title'], $post['thread_title'], $vbulletin->forumcache["$post[forumid]"]['title']));
		}
...
if ($_POST['do'] == 'domergeposts')
{
	...
	if(!in_array($vbulletin->GPC['postid'],$postids)){
			array_push($postids,$vbulletin->GPC['postid']);
	}
	...
	{
		return_mod_fault(fetch_error('invalidid', $vbphrase['post'], $vbulletin->options['contactuslink']));
	}
	...
	{
		return_mod_fault(fetch_error('guest_posts_may_not_contain_attachments'));
	}
...
if ($_POST['do'] == 'domoveposts')
{
	...
	{	// Create a new thread
        ...
        $title = mobiquo_encode($destthreadinfo['title'], 'to_local');
		$threadman->setr('title', $title, true, false);
...
if ($_POST['do'] == 'renamethread')
{
    ...
    // Validate threads
    $threads = $db->query_read_slave("
        SELECT threadid, visible, open, forumid, postuserid, firstpostid, prefixid
        FROM " . TABLE_PREFIX . "thread
        WHERE threadid IN($threadids)
    ");
    while ($thread = $db->fetch_array($threads))
    {
        ...
        //can_rename
    	$mobiquo_can_rename = false;
	    if (can_moderate($thread['forumid'], 'canmoderateposts'))
	        $mobiquo_can_rename = true;
	    else if( !can_moderate($thread['forumid'], 'canmoderateposts') AND
	        !can_moderate($thread['forumid'], 'caneditposts')          AND
	        $thread['open'] AND ($forumperms & $vbulletin->bf_ugp_forumpermissions['caneditpost']))
	    {

	        $first_post = $db->query_first("
	                    SELECT post.* FROM " . TABLE_PREFIX . "post as post
	                    WHERE post.threadid = ".$thread['threadid']." and post.parentid = 0 ORDER BY post.dateline");
	       if  (
	            $vbulletin->userinfo['userid'] == $first_post['userid'] AND
	            ($vbulletin->options['edittimelimit'] == 0 OR ($first_post['dateline'] >= (TIMENOW - $vbulletin->options['edittimelimit'] * 60) AND $vbulletin->options['edittimelimit'] != 0))
	           )
	        {
	            $mobiquo_can_rename = true;
	        }
    ...
    {
        $prefixid = ($vbulletin->GPC_exists['prefixid'] AND can_use_prefix($vbulletin->GPC['prefixid'])) ? $vbulletin->GPC['prefixid'] : $thread['prefixid'];
        $foruminfo = fetch_foruminfo($thread['forumid']);
        if ($prefixid === '' AND ($foruminfo['options'] & $vbulletin->bf_misc_forumoptions['prefixrequired']))
        {
            // the prefix wasn't valid or was set to an empty one, but that's not allowed
            return_mod_fault(fetch_error('thread_prefix_required'));
        }
    }
    ...
    $db->query_write("
        UPDATE " . TABLE_PREFIX . "thread
        SET title = '" . $db->escape_string($vbulletin->GPC['title']) . "'".(isset($prefixid)? ",prefixid = '".$db->escape_string($vbulletin->GPC['prefixid'])."'" : "")."
        WHERE threadid IN($threadids)
    ");
    ...
    $db->query_write("
        ...
        WHERE threadid IN($threadids) AND parentid = 0
    ");
