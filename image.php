...
// ######################### REQUIRE BACK-END ############################
if (!empty($_REQUEST['type']) AND $_REQUEST['type'] == 'dberror') // do not require back-end
{
	header('Content-type: image/jpeg');
	readfile('./includes/database_error_image.jpg');
	exit;
}
else if (!empty($_REQUEST['type']) AND $_REQUEST['type'] == 'ieprompt')
{
	header('Content-type: image/jpeg');
	readfile('./includes/ieprompt.jpg');
	exit;
}
else if (!empty($_REQUEST['type']) AND $_REQUEST['type'] == 'profile') // do not modify this $_REQUEST
{
