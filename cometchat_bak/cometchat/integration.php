...
define('DB_AVATARTABLE',		" LEFT JOIN ".TABLE_PREFIX."customprofilepic as customprofilepic ON(customprofilepic.userid = user.userid) LEFT JOIN ".TABLE_PREFIX."customavatar as customavatar ON(customavatar.userid = user.userid) ");
define('DB_AVATARFIELD',                "CONCAT( user.userid,'^',COALESCE( customprofilepic.dateline,''),'^',COALESCE(customavatar.dateline,''),'^',COALESCE(NOT ISNULL(customprofilepic.userid),''),'^', COALESCE(NOT ISNULL(customavatar.userid),''),'^',COALESCE(user.email,'') )");
...
function getFriendsList($userid,$time) {
	...
	$sql = "SELECT awesome.id userid, concat(MAX( cometchat.sent ),'^', cometchat.message) message ,".DB_USERTABLE.'.'.DB_USERTABLE_NAME." username,  ".DB_USERTABLE.'.'.DB_USERTABLE_NAME." link,  ".DB_AVATARFIELD."  avatar,cometchat_status.lastactivity lastactivity, cometchat_status.status,  cometchat_status.isdevice
			FROM (
				...
				GROUP BY id
			)awesome, cometchat,cometchat_status,".DB_USERTABLE.DB_AVATARTABLE."
			WHERE awesome.msg = cometchat.id and awesome.id = ".DB_USERTABLE.'.'.DB_USERTABLE_USERID." and ".DB_USERTABLE.'.'.DB_USERTABLE_USERID." = cometchat_status.userid and (cometchat_status.status IS NULL OR cometchat_status.status <> 'invisible' OR cometchat_status.status <> 'offline')
			GROUP BY awesome.id
