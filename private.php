...
{
	$temppmid = !empty($_REQUEST['pmid']) ? intval($_REQUEST['pmid']) : 0;
	$temppmid = ($temppmid) < 0 ? 0 : $temppmid;
...
$_POST['do'] = isset($_POST['do']) ? $_POST['do'] : '';
...
if ($_POST['do'] == 'updatefolders')
{
	...
	{
		$oldpmfolders = vb_unserialize($vbulletin->userinfo['pmfolders']);
		$pmfolders = array();
		$updatefolders = array();
...
if ($_REQUEST['do'] == 'emptyfolder')
{
	...
	{
		$pmfolders = $pmfolders + vb_unserialize($vbulletin->userinfo['pmfolders']);
	}
...
if ($_REQUEST['do'] == 'editfolders')
{
	if (!isset($pmfolders))
	{
		$pmfolders = vb_unserialize($vbulletin->userinfo['pmfolders']);
	}
...
if ($_POST['do'] == 'movepm')
{
	...
	$vbulletin->GPC['messageids'] = vb_unserialize(verify_client_string($vbulletin->GPC['messageids']));
	...
	switch($vbulletin->GPC['dowhat'])
	{
		...
		case 'move':
			...
			switch ($vbulletin->GPC['folderid'])
			{
				...
				default:
				{
					$folders = vb_unserialize($vbulletin->userinfo['pmfolders']);
...
if ($_REQUEST['do'] == 'downloadpm')
{
	...
	function fetch_touser_string($pm)
	{
		...
		$bcclist = array();
		$ccrecipients = '';
		$touser = vb_unserialize($pm['touser']);
...
if ($_REQUEST['do'] == 'showpm')
{
	...
	$ccrecipients = '';
	$bccrecipients = '';
	$touser = vb_unserialize($pm['touserarray']);
	if (!is_array($touser))
	{
		$touser = array();
	}
...
if ($_REQUEST['do'] == 'messagelist')
{
	...
	$totalmessages = isset($messagecounters["{$vbulletin->GPC['folderid']}"]) ? intval($messagecounters["{$vbulletin->GPC['folderid']}"]) : 0;
		...
		foreach ($pm_period_groups AS $groupid => $pms)
		{
			...
			foreach ($pms AS $pmid => $pm)
			{
				...
				else
				{
					...
					{
						$users = vb_unserialize($pm['touserarray']);
						$touser = array();
						$tousers = array();
						if (!empty($users))
						{
