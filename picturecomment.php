...
if ($_REQUEST['do'] == 'viewip')
{
	...
	$commentinfo['hostaddress'] = @gethostbyaddr($commentinfo['ipaddress']);

	($hook = vBulletinHook::fetch_hook('picture_comment_getip')) ? eval($hook) : false;

	standard_error(fetch_error('thread_displayip', $commentinfo['ipaddress'], htmlspecialchars_uni($commentinfo['hostaddress'])), '', 0);
}
